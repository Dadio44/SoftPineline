...
static void create_surface(HWND handle, int width, int height,
    IRenderTarget**out_surface, HDC *out_memory_dc) {
    BITMAPINFOHEADER bi_header;
    HBITMAP dib_bitmap;
    HBITMAP old_bitmap;
    HDC window_dc;
    HDC memory_dc;
    IRenderTarget* surface;

    surface = new BMP::BMP();
    surface->SetSize(width, height);
    /*free(surface->GetBitmapBuffer());
    ...
    dib_bitmap = CreateDIBSection(memory_dc, (BITMAPINFO*)&bi_header,
                                  DIB_RGB_COLORS, (void**)surface->GetBitmapBufferAddress(),
                                  NULL, 0);
...
void window_destroy(window_t *window) {
    ...
    window->surface->SetBitmapBuffer(nullptr);
...
static void present_surface(window_t *window) {
    ...
    IRenderTarget*surface = window->surface;
